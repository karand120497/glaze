...
Glaze  has access to the following tools:"""

GLAZE_FORMAT_INSTRUCTIONS = """To use a tool, please use the following format:
...
GLAZE_SUFFIX = """You are very strict to the filename correctness and will never fake a file name if it does not exist.
...
class Text2manga:
    def __init__(self, device):
        print(f"Initializing Text2Image_manga to {device}")
        self.device = device
        self.torch_dtype = torch.float16 if 'cuda' in device else torch.float32
        self.pipe = StableDiffusionPipeline.from_pretrained("hakurei/waifu-diffusion",
                                                            torch_dtype=self.torch_dtype)
        self.pipe.to(device)
        self.a_prompt = 'best quality, extremely detailed'
        self.n_prompt = 'longbody, lowres, bad anatomy, bad hands, missing fingers, extra digit, ' \
    ...
    @prompts(name="Generate manga image from user input",
             description="useful when you want to generate a manga image from a user input text and save it to a file. "
                         "like: generate an image of an object or something in manga style, or generate an image that includes some objects in manga style. "
                         "The input to this tool should be a string, representing the text used to generate image. ")
    def inference(self, text):
        image_filename = os.path.join('image', f"{str(uuid.uuid4())[:8]}.png")
        prompt = text + ', ' + self.a_prompt
        with autocast("cuda"):
          image = self.pipe(prompt,negative_prompt=self.n_prompt, guidance_scale=6)["sample"][0]
        #image = self.pipe(prompt, negative_prompt=self.n_prompt, guidance_scale=6)["sample"][0]
        #image = self.pipe(prompt, negative_prompt=self.n_prompt).images[0]
        image.save(image_filename)
        print(
            f"\nProcessed Text2Image_manga, Input Text: {text}, Output Image: {image_filename}")
        return image_filename

class borders:
    def __init__(self, device):
        print(f"Initializing borders to {device}")
        self.device = device
        self.torch_dtype = torch.float16 if 'cuda' in device else torch.float32
        #self.model_id = ""
        #self.pipe = StableDiffusionPipeline.from_pretrained(self.model_id,torch_dtype=self.torch_dtype)
        #self.pipe.to(device)
        #self.a_prompt = 'best quality, extremely detailed'
    ...
    @prompts(name="Add borders to image",
             description="Useful when user wants to edit images by changing or adding a specific type of border to them, or when user wants the image in a specific size or border width. Borders can be useful for framing images or drawing attention to specific parts of them. Comment end  ")
    def inference(self, inputs):
        image_path, text = inputs.split(",")[0], ','.join(inputs.split(',')[1:])
        ...
        image = cv2.copyMakeBorder(src, top, bottom, left, right, borderType, None, value)    
        #image = self.pipe(prompt, negative_prompt=self.n_prompt).images[0]
        image.save(image_filename)
        print(
...
class ConversationBot:
    def __init__(self, load_dict):
        ...
        print(f"Initializing glaze, load_dict={load_dict}")
        if 'ImageCaptioning' not in load_dict:
            raise ValueError("You have to load ImageCaptioning as a basic function for glaze")
        ...
        self.agent = initialize_agent(
            ...
            agent_kwargs={'prefix': GLAZE_PREFIX, 'format_instructions': GLAZE_FORMAT_INSTRUCTIONS,
                          'suffix': GLAZE_SUFFIX}, )
...
if __name__ == '__main__':
    ...
    with gr.Blocks(css="#chatbot .overflow-y-auto{height:500px}") as demo:
        chatbot = gr.Chatbot(elem_id="chatbot", label="Glaze")
        state = gr.State([])
        with gr.Row():
        ...
        demo.launch(server_name="0.0.0.0", server_port=1015,share=True)
